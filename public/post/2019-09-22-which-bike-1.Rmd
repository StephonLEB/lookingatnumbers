...
The max number of activities per API request is 200, so i needed to make a few requests to get a large number of rides. Combinining the two into a single data frame wasn't trivial, as the tables had a different number of variables.
...
library(cowplot)
rides %>% ggplot(aes(y=average_watts,x=gear_id)) + geom_boxplot() ->p1
